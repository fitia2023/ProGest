...
public class AnneeController {
    ...
    public ResponseEntity<Annee> idannee(@PathVariable Long id) {
        Annee annee = anneeRepo.findById(id)
                .orElseThrow(() -> new NotFound("Annee n'existe pas ou pas trouvé"));
    ...
    public ResponseEntity<Annee> upid(@PathVariable Long id, @RequestBody Annee annee) {
        Annee anneeup = anneeRepo.findById(id)
                .orElseThrow(() -> new NotFound("annee n'existe pas encore"));
    ...
    public ResponseEntity<Map<String, Boolean>> delis(@PathVariable Long id) {
        Annee annee = anneeRepo.findById(id)
                .orElseThrow(() -> new NotFound("Annee n'exoiste pas"));
     ...
     @GetMapping("look/{ans}/{id_projet}")
     public Annee obteniranneeprojet(@PathVariable Integer ans ,@PathVariable Long id_projet){
        return anneeRepo.findByAnsAndIdProjet(ans,id_projet);
     }

     @GetMapping("list/{id_projet}")
    public List<Annee> obtenirlistanneeparid(@PathVariable Long id_projet){
        return anneeRepo.findByProjetId(id_projet);
     }
}
